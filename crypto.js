...
export async function derivePairwiseKey(sharedSecret, idA, idB) {
  const pair = [idA, idB].sort().join(':');
  const digest = await sha256Bytes(`${sharedSecret}:${pair}:dm-v1`);
...
export async function encryptMessage(key, payload) {
  ...
  return { iv: bufferToBase64(iv), cipher: bufferToBase64(new Uint8Array(cipher)) };
}
